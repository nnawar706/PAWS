...
if(isset($_POST['submit'])) {
  ...
  $query = mysqli_query($con,"insert into users(name,email,contact,address,password) values('$name','$email','$contact','$address','$password')");
  if($query){
    echo "<script>alert('Congratulations! You have successfully registered!');</script>";
  }
  else{
    echo "<script>alert('Caution: Not registered yet. Something went wrong');</script>";
  }
}
...
if(isset($_POST['signin'])){
  ...
  if($num>0){
    ...
    $status = 1;
    $log = mysqli_query($con,"insert into userlog(email,userip,status) values('".$_SESSION['signin']."','$uip','$status')");
    ...
    $uip=$_SERVER['REMOTE_ADDR'];
    $status=0;
    $log=mysqli_query($con,"insert into userlog(email,userip,status) values('$email','$uip','$status')");
    $host  = $_SERVER['HTTP_HOST'];
    $uri  = rtrim(dirname($_SERVER['PHP_SELF']),'/\\');
    $extra="signup.php";
    header("location:http://$host$uri/$extra");
...
<head>
  ...
  <title>PAWS - Sign Up|Sign In</title>
                ...
                <div class="inner-box" id="card">
                  ...
                  <div class="card-back">
                    ...
                    <form role="form" method="post" name="register" onSubmit="return valid();">
                      ...
                      <input type="text" name="address" class="input-box" placeholder="Enter Your Current Address" required>
