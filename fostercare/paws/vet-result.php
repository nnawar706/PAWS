...
if(isset($_POST['submit'])){
	...
	$email=$_POST['email'];
	$nid=$_POST['nid'];
	$query=mysqli_query($con,"select * from appliedvets where nid='$nid'");
	$num=mysqli_fetch_array($query);
	if($num>0){
		$val1='Not Approved Yet';
		$val2='Declined';
		$val3='Approved';
		if($num['status']==$val1){
			echo "<script>alert('Your request has not been approved yet. please wait until our admins review your application. Thank you.');</script>";
		}
		else if($num['status']==$val2){
			echo "<script>alert('Your application unfortunately does not fulfill our requirements. Try one more later. Thank you.');</script>";
		}
		else if($num['status']==$val3){
			echo "<script>alert('Congratulations! Your application has been approved. You may now sign up to PAWS as a vet.');</script>";
		}
	}
	else{
		echo "<script>alert('You have not applied for the post yet.');</script>";
		}
}
?>
          ...
          <div class="col-lg-4">
            ...
            <div class="info">
              ...
              <div class="info-item d-flex">
                ...
                <div>
                  <a href="check-result.php"><h4>Check Result</h4></a>
                </div>
              </div><!-- End Info Item -->
